        ...
        for (const phaseId of phaseIds) {
            ...
            for (const moveId of phase.moves) {
                if (!knownMoves.has(moveId)) {
                    throw new Error(`defineGame: phase "${phaseId}" references unknown move "${moveId}"`);
                }
            }
            if (phase.next !== undefined && !knownPhases.has(phase.next)) {
                throw new Error(`defineGame: phase "${phaseId}" next references unknown phase "${phase.next}"`);
            }
