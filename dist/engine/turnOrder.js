 ...
 */
export function nextPlayer(ctx, order) {
    const { numPlayers, currentPlayer } = ctx;
    if (!Number.isInteger(numPlayers) || numPlayers < 1) {
        throw new TurnOrderError(`nextPlayer: numPlayers must be a positive integer (got ${String(numPlayers)})`);
    }
    if (order === undefined || order === 'sequential') {
        return (currentPlayer + 1) % numPlayers;
    }
    if (typeof order !== 'function') {
        throw new TurnOrderError(`nextPlayer: unknown turn order strategy ${String(order)}`);
    }
    const next = order(ctx);
    if (!Number.isInteger(next) || next < 0 || next >= numPlayers) {
        throw new TurnOrderError(`nextPlayer: order function returned ${String(next)}, ` +
