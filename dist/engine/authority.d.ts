...
/**
 * Result of {@link validateMove}: a discriminated union mirroring the engine's
 * `{ ok: true, ... }` / `{ ok: false, reason }` style.
 *
 * On success it carries `nextState`, the match state RECOMPUTED on the server
 * from `prevMatch` (never from any client-supplied state).
 *
 * @typeParam G - the game-specific state shape.
 */
export type ValidateResult<G> = {
    readonly ok: true;
 ...
 *          `{ ok: false, reason }`.
 */
export declare function validateMove<G>(def: GameDefinition<G>, prevMatch: MatchState<G>, action: Action, playerId: number): ValidateResult<G>;
