...
export function createMatch<G>(
  ...
  if (numPlayers < def.turn.minPlayers || numPlayers > def.turn.maxPlayers) {
    throw new Error(
      `createMatch: numPlayers ${numPlayers} out of range ` +
 ...
 */
function isMoveAllowedInPhase<G>(
  def: GameDefinition<G>,
  phase: string | null,
  moveId: string,
): boolean {
  if (phase === null || def.phases === undefined) return true;
  const phaseConfig = def.phases[phase];
  if (phaseConfig === undefined) return false;
 ...
 * stays `null`.
 */
function phaseAfterTurn<G>(
  def: GameDefinition<G>,
  phase: string | null,
): string | null {
  if (phase === null || def.phases === undefined) return phase;
  const phaseConfig = def.phases[phase];
  if (phaseConfig === undefined) return phase;
